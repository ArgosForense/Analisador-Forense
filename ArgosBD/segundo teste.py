# Adicione esta linha para instalar a dependência no ambiente Colab
!pip install pymongo
...
MONGO_URI = "mongodb+srv://analistaforense:OaQKatRGsPNQS2rZ@cluster0.bqlrw4x.mongodb.net/ArgosDB?retryWrites=true&w=majority"
DB_NAME = "ArgosDB"
...
def run_crud_simulation(db):
    ...
    # Adicionar um novo usuário para teste
    novo_usuario_teste = db['USUARIOS'].insert_one({
        "nome": "Novo Usuario Teste",
        "email": "novo.teste@simulacao.com",
        "senha": "senha_nova",
        "empresa_id": nova_empresa.inserted_id, # Associar ao ID da empresa criada
        "perfil_id": PERFIL_FUNCIONARIO_ID, # Usar o ID de perfil funcionário
        "status": True
    })
    temp_ids.append(('USUARIOS', novo_usuario_teste.inserted_id))
    print(f"   - NOVO USUARIO DE TESTE criado. ID: {novo_usuario_teste.inserted_id}")
    ...
    # Buscar o novo usuário de teste criado
    usuario_teste_lido = db['USUARIOS'].find_one({"_id": novo_usuario_teste.inserted_id})
    if usuario_teste_lido:
        print(f"   - Novo usuário de teste lido com sucesso (Nome: {usuario_teste_lido['nome']})")
    else:
        print("   - Falha ao ler o novo usuário de teste.")
